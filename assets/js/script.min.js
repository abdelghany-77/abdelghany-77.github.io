...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const openFullscreen = function(carouselId, index = null) {
    ...
    if (index === null) {
      currentIndex = Array.from(currentCarousel.querySelectorAll('.carousel-item'))
  ...
  document.addEventListener('keydown', function(event) {
    if (event.key === 'Escape' && modal.classList.contains('active')) {
      closeFullscreen();
    } else if (event.key === 'ArrowLeft' && modal.classList.contains('active')) {
      navigateImages(-1);
    } else if (event.key === 'ArrowRight' && modal.classList.contains('active')) {
      navigateImages(1);
    }
  });
